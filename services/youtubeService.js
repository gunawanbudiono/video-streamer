...
async function endYouTubeBroadcast(streamId) {
  try {
    const stream = await Stream.findById(streamId);
    if (!stream || !stream.is_youtube_api || !stream.youtube_broadcast_id) {
      return { success: true, message: 'Not a YouTube API broadcast' };
    }

    const user = await User.findById(stream.user_id);
    if (!user || !user.youtube_client_id || !user.youtube_client_secret) {
      return { success: false, error: 'Missing YouTube API credentials' };
    }

    const selectedChannel = await YoutubeChannel.findById(stream.youtube_channel_id);
    if (!selectedChannel || !selectedChannel.access_token) {
      return { success: false, error: 'Missing channel tokens' };
    }
    ...
    console.log(`[YouTubeService] Transitioning broadcast ${stream.youtube_broadcast_id} to complete...`);
    await youtube.liveBroadcasts.transition({
      broadcastStatus: 'complete',
      id: stream.youtube_broadcast_id,
      part: 'id,status'
    });
    console.log(`[YouTubeService] Broadcast ${stream.youtube_broadcast_id} transitioned to complete successfully`);
    return { success: true };
  } catch (error) {
    console.log('[YouTubeService] Note on transition broadcast to complete:', error.message);
    return { success: false, error: error.message };
  }
}
